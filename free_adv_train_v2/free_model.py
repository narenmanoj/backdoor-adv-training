...
class Model(object):
    ...
    def _build_model(self):
        ...
        with tf.compat.v1.variable_scope('input'):
            ...
            if self.pert:
                self.pert = tf.compat.v1.get_variable(name='instance_perturbation', initializer=tf.compat.v1.zeros_initializer,
        ...
        self.num_correct = tf.reduce_sum(input_tensor=tf.cast(self.correct_prediction, tf.int64))
    ...
    def _batch_norm(self, name, x):
        ...
        with tf.compat.v1.name_scope(name):
            return tf.compat.v1.layers.batch_normalization(inputs=x, momentum=.9, center=True, scale=True, training=(self.mode == 'train'))
